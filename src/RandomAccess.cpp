...
public:
  ...
  int jump(const std::vector<std::string>& filenames, const std::vector<int64_t> &offsets, uintptr_t runtime, uintptr_t ctx) {
    _pieces.reset();
...
int RandomAccess::jump(const std::vector<std::string>& filenames, const std::vector<int64_t> &offsets, uintptr_t runtime, uintptr_t ctx) {
  return _raxrun->jump(filenames, offsets, runtime, ctx);
}
